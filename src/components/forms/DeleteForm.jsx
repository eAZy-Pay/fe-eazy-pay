...
const DeleteForm = ({ id, pageName, name, fetchRows }) => {
  ...
  const deleteData = () => {
    ...
    if (pageName === 'QnA') {
      deleteQna(id).then((res) => handleDeleteResponse(res));
    }
  };
  ...
  const handleDeleteResponse = (res) => {
    console.log(res);
    if (res.status === 200) {
      alert(`${pageName} 삭제에 성공했습니다.`);
    } else {
      alert(`${pageName} 삭제에 실패했습니다.`);
    }
    fetchRows();
  };
